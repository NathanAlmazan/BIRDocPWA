...
function NotificationItem({ userId, notification, onClose }: { userId: string, notification: Thread, onClose: () => void }) {
    ...
    const handleRedirect = (threadId: string) => {
      if (notification.author.accountId === userId) {
        if (!notification.completed && notification.actionable) navigate(`/app/sent/pending/${threadId}`);
        else if (notification.completed) navigate(`/app/sent/memos/${threadId}`)
        else if (notification.completed && notification.actionable) navigate(`/app/sent/completed/${threadId}`)
      } else {
        if (!notification.completed && notification.actionable) navigate(`/app/inbox/tasks/${threadId}`);
        else if (notification.completed) navigate(`/app/inbox/memos/${threadId}`)
        else if (notification.completed && notification.actionable) navigate(`/app/inbox/finished/${threadId}`)
      }
      onClose();
    }
